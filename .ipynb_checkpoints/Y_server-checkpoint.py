...
# from engine_simple_infer_STANDARD_VGG_classification_101 import *
...
# local version
# checkpoints
CHECKPOINT_FILE = 'C:/Users/Alex Chen/Google Drive (yoyohc10@g.ucla.edu)/CamiotEnv/Fall 2020 Work/New Code/User Study/model_1.pth'
...
def cut_query_img_with_finger(img):
    ...
    image_canny_filter = cv2.Canny(image=img.astype('uint8'), threshold1=20, threshold2=40)
    ...
    # cv2.imwrite('/home/yuan/CAMIOT/recognition_conv/mask.jpg', image_sobel*255)
    if np.count_nonzero(image_sobel) <= NO_FIGURE_PIXEL_NUM_THRESHOLD:
        ...
        if bot_line_index <= 0.95 * img.shape[0]:
            ...
            left_y = max(0, int(mid_y - 0.5 * img.shape[1]))
            right_y = min(img.shape[1], int(mid_y + 0.5 * img.shape[1]))
            if top_line_index - int(0.8 * img.shape[0]) >= 0:
                upper_line = top_line_index - int(0.8 * img.shape[0])
            else:
                upper_line = 0
            return img[upper_line:max(img.shape[0], top_line_index+int(0.2 * img.shape[0])), left_y:right_y, :]
...
def appliance_recognition(cur_img, cur_model):
    # load image
    cur_img_cp = cur_img.copy()
    cur_img = cut_query_img_with_finger(cur_img)
    if (cur_img.shape[0] * cur_img.shape[1]) <= (cur_img_cp.shape[0] * cur_img_cp.shape[1]) * 0.1:
        cur_img = cur_img_cp

    # print(cur_img.shape)
    # cv2.imwrite('/home/yuan/CAMIOT/recognition_conv/cropped.jpg', cur_img)

    cur_img, scale_factor = imrescale(
        img=cur_img,
        scale=IMG_SCALES,
        return_scale=True,
        interpolation='bilinear',
    )
    cur_img = _impad(img=cur_img, shape=(IMG_SCALES[1], IMG_SCALES[0]), pad_val=(0, 0, 0))
    cur_img = imnormalize(cur_img, NORM_CONFIG['mean'], NORM_CONFIG['std'], NORM_CONFIG['to_rgb']).astype(
        np.float32)
    cur_img = torch.Tensor(np.expand_dims(cur_img.transpose(2, 0, 1), 0), device=torch.device('cpu'))

    # model run
    with torch.no_grad():
        result_prob = cur_model(
            x=cur_img,
        )
...
img = picRecv(conn0,0,0)
time.sleep(0.5)
result_prob, result_items = appliance_recognition(img, model)
print('Finished appliance recognition')
print(result_items)
#send list to 
pack = pickle.dumps(result_items)
conn0.sendall(pack)
